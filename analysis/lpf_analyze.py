...
def grab_raw_data(uri = '/home/dbcometto/workspace/wifi_ws/src/wifi_localization/bags/0_0_0_11-17', storage_id='mcap', msg_type = WifiList):
    all_data = []
...
def collect_raw_data(data_path='/home/dbcometto/workspace/wifi_ws/src/wifi_localization/bags/0_0_0_11-17',start_time = 0, stop_time =100):
    my_data = grab_raw_data(data_path)
...
def analyze_data(folder_path = '/home/dbcometto/workspace/wifi_ws/src/wifi_localization/bags',
    ...
    data_path  = f"{folder_path}/ben_tests/lpf_sim_test"

    raw_elapsed_time,raw_wifi_data,raw_unique_bssid_list,raw_unique_bssid_count,raw_rssis = collect_raw_data(data_path=data_path, start_time=start_time, stop_time=stop_time)
    ...
    bssid_to_index_1 = {bssid: i for i, bssid in enumerate(unique_bssid_list)}

    # Build a mapping from index in 0 -> index in 1
    index_mapping = {}
    for i, bssid in enumerate(raw_unique_bssid_list):
    ...
    raw_indices,indices = zip(*index_mapping.items())



    fs = 0.2
    raw_k = np.shape(raw_rssis)[1]
    raw_df = fs/raw_k
    raw_freqs = raw_df*np.arange(raw_k)
    raw_fft = np.abs(np.fft.fft(raw_rssis))
    raw_fft_db = 20*np.log10(raw_fft/np.max(raw_fft,axis=1)[:,None])
    ...
    for j in range(num_to_compare):
        i = raw_indices[j]
        one_bssid_rssi = raw_rssis[i,:]
        axs[r,c].plot(raw_elapsed_time,one_bssid_rssi,marker=".",linestyle='-',label=f"BSSID {i}",zorder=1)
    ...
    for j in range(num_to_compare):
        i = indices[j]
        one_bssid_rssi = rssis[i,:]
        axs[r,c].plot(elapsed_time,one_bssid_rssi,marker=".",linestyle='-',label=f"Filtered BSSID {i}",zorder=1)
    ...
    for j in range(np.shape(rssis)[0]):
        i = indices[j]
        one_bssid_fft = raw_fft_db[i,:]
        axs[r,c].plot(raw_freqs,one_bssid_fft,marker=".",linestyle='-',label=f"BSSID {i}",zorder=1)
    ...
    for j in range(np.shape(rssis)[0]):
        i = indices[j]
        one_bssid_fft = fft_db[i,:]
        axs[r,c].plot(freqs,one_bssid_fft,marker=".",linestyle='-',label=f"Filtered BSSID {i}",zorder=1)
...
analyze_data(fig_title  = "LPF Performance",
             start_time = 0,
             stop_time  = 10000)
